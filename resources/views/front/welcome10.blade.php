            ...
            <div class="map map-image" id="map">
                ...
                <img src="{{$scheme->image}}"/>
                    ...
                    @foreach($scheme->store as $store)
                        ...
                        <div style="position: absolute; left: {{$store->x_01}}px; top: {{$store->y_01}}px; z-index:10; width: auto; height: auto; margin-left: -12px; margin-top: -12px;"><a href="/front-r10routes/@foreach($store->r10routes as $route){{$route->id}}@endforeach" style="display: block; padding: 5px; text-decoration: none; color: #fff; font-size: 10px; background: #ee3984; border-radius: 5px; box-shadow: 0 1px 16px rgba(0,0,0,0.5);">{{$store->title}}</a></div>
                        @endif
                    @endforeach
                </div>
                @endforeach
...
<div class="search">
    <a href="/search10"><img src="/img/search.png"><span>Поиск</span></a>
</div>
